...
const removeWishlistProduct = asyncHandler (async (req,res) => {
     try{
          ...
          await wishlistCollection.updateOne({user:req.session.user._id},{
               $pull:{products:proId}
          })
          res.redirect("/products/wishlist")
...
const productsFilter = asyncHandler(async(req,res)=>{
    ...
    const product = await productCollection.find({category:{$in:req.body.category}}).sort({price:sort}).lean()
...
const deleteImage = asyncHandler(async(req,res)=>{
     if(req.query.type=='main'){
          ...
          await productCollection.updateOne({_id:req.query.proId},{
              $unset:{image:1}
          })
          res.json({success:true})
     }
...
if(products.unlist==false){
     await productCollection.updateOne({_id:req.query.id},{
          $set:{
               unlist:true
          }
     })
